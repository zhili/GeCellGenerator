...
namespace msexcel {
	...
	int excelreader::rowCount() {
		...
		_variant_t result;
		AutoWrap(DISPATCH_PROPERTYGET,&result, range.pdispVal, L"Rows", 0);
		_variant_t rows;
 ...
 /**
    ...
    std::wstring excelreader::GetColumnName(int nIndex)
    {
    ...
    bool excelreader::CheckFilename(const std::wstring& filename)
    {
        if (filename.empty())
        {
            return false;
    ...
    void excelreader::AutoWrap(int autoType, VARIANT* pvResult,
                                IDispatch *pDisp,
